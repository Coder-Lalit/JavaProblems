package LeetCode.Problems.AugustLeetCodingChallenge2021;

import org.testng.annotations.Test;

import java.util.ArrayList;
import java.util.HashMap;
import java.util.List;

//Not a optimal solution
public class MaximumProfitInJobScheduling1235 {
    static int max;

    public static int jobScheduling1(int[] s, int[] e, int[] p) {
        int maxE = Integer.MIN_VALUE;
        for (int i = 0; i < s.length; i++) {
            maxE = Math.max(maxE, e[i]);
        }
        int jobMat[][] = new int[maxE + 1][maxE + 1];

        for (int i = 0; i < s.length; i++) {
            jobMat[s[i]][e[i]] = p[i];
        }

        for (int[] x : jobMat) {
            for (int y : x) {
                System.out.print(y + " ");
            }
            System.out.println();
        }

        max = Integer.MIN_VALUE;
        return getPossiblePath1(jobMat, 1, "", 0);
    }

    static int getPossiblePath1(int[][] m, int s, String path, int p) {
        String prvPath = path;
        int prvP = p;
        for (; s < m.length; s++) {
            for (int j = s; j < m[0].length; j++) {
                path = prvPath;
                p = prvP;
                if (m[s][j] != 0) {
                    p += m[s][j];
                    path += m[s][j] + "(" + p + ") ->";
                    max = Math.max(max, p);
                    System.out.println(path);
                    getPossiblePath1(m, j, path, p);
                }
            }
        }
        return max;
    }


    public static int jobScheduling(int[] s, int[] e, int[] p) {
        HashMap<Integer, List<Integer>> sMap = new HashMap<>();
        int maxE = Integer.MIN_VALUE;
        for (int i = 0; i < s.length; i++) {
            maxE = Math.max(maxE, e[i]);
            if (!sMap.containsKey(s[i])) {
                sMap.put(s[i], new ArrayList<>());
            }
            sMap.get(s[i]).add(e[i]);
        }
        int jobMat[][] = new int[maxE + 1][maxE + 1];

        for (int i = 0; i < s.length; i++) {
            jobMat[s[i]][e[i]] = p[i];
        }

        max = Integer.MIN_VALUE;
        return getPossiblePath(jobMat, sMap, 1, 0);
    }

    static int getPossiblePath(int[][] m, HashMap<Integer, List<Integer>> sMap, int s, int p) {
        int prvP = p;
        for (; s < m.length; s++) {
            if (sMap.containsKey(s)) {
                for (int j : sMap.get(s)) {
                    p = prvP;
                    p += m[s][j];
                    max = Math.max(max, p);
                    getPossiblePath(m, sMap, j,  p);

                }
            }
        }
        return max;
    }


    public static void main(String[] args) {
        //jobScheduling(new int []{1,2,3,3},new int[]{3,4,5,6},new int []{50,10,40,70});
        //jobScheduling(new int[]{1, 1, 1, 3, 3, 3, 4, 4, 5, 6}, new int[]{2, 4, 6, 4, 5, 7, 5, 7, 6, 7}, new int[]{20, 50, 80, 90, 80, 70, 10, 60, 20, 80});
        int i = jobScheduling(new int[]{341, 22, 175, 424, 574, 687, 952, 439, 51, 562, 962, 890, 250, 47, 945, 914, 835, 937, 419, 343, 125, 809, 807, 959, 403, 861, 296, 39, 802, 562, 811, 991, 209, 375, 78, 685, 592, 409, 369, 478, 417, 162, 938, 298, 618, 745, 888, 463, 213, 351, 406, 840, 779, 299, 90, 846, 58, 235, 725, 676, 239, 256, 996, 362, 819, 622, 449, 880, 951, 314, 425, 127, 299, 326, 576, 743, 740, 604, 151, 391, 925, 605, 770, 253, 670, 507, 306, 294, 519, 184, 848, 586, 593, 909, 163, 129, 685, 481, 258, 764},
                new int[]{462, 101, 820, 999, 900, 692, 991, 512, 655, 578, 996, 979, 425, 893, 975, 960, 930, 991, 987, 524, 208, 901, 841, 961, 878, 882, 412, 795, 937, 807, 957, 994, 963, 716, 608, 774, 681, 637, 635, 660, 750, 632, 948, 771, 943, 801, 985, 476, 532, 535, 929, 943, 837, 565, 375, 854, 174, 698, 820, 710, 566, 464, 997, 551, 884, 844, 830, 916, 970, 965, 585, 631, 785, 632, 892, 954, 803, 764, 283, 477, 970, 616, 794, 911, 771, 797, 776, 686, 895, 721, 917, 920, 975, 984, 996, 471, 770, 656, 977, 922},
                new int[]{85, 95, 14, 72, 17, 3, 86, 65, 50, 50, 42, 75, 40, 87, 35, 78, 47, 74, 92, 10, 100, 29, 55, 57, 51, 34, 10, 96, 14, 71, 63, 99, 8, 37, 16, 71, 10, 71, 83, 88, 68, 79, 27, 87, 3, 58, 56, 43, 89, 31, 16, 9, 49, 84, 62, 30, 35, 7, 27, 34, 24, 33, 100, 25, 90, 79, 58, 21, 31, 30, 61, 46, 36, 45, 85, 62, 91, 54, 28, 63, 50, 69, 48, 36, 77, 39, 19, 97, 20, 39, 48, 72, 37, 67, 72, 46, 54, 37, 53, 30});
        System.out.println(i);
    }

    @Test
    public void myTest(){
        int i = jobScheduling(new int[]{341, 22, 175, 424, 574, 687, 952, 439, 51, 562, 962, 890, 250, 47, 945, 914, 835, 937, 419, 343, 125, 809, 807, 959, 403, 861, 296, 39, 802, 562, 811, 991, 209, 375, 78, 685, 592, 409, 369, 478, 417, 162, 938, 298, 618, 745, 888, 463, 213, 351, 406, 840, 779, 299, 90, 846, 58, 235, 725, 676, 239, 256, 996, 362, 819, 622, 449, 880, 951, 314, 425, 127, 299, 326, 576, 743, 740, 604, 151, 391, 925, 605, 770, 253, 670, 507, 306, 294, 519, 184, 848, 586, 593, 909, 163, 129, 685, 481, 258, 764},
                new int[]{462, 101, 820, 999, 900, 692, 991, 512, 655, 578, 996, 979, 425, 893, 975, 960, 930, 991, 987, 524, 208, 901, 841, 961, 878, 882, 412, 795, 937, 807, 957, 994, 963, 716, 608, 774, 681, 637, 635, 660, 750, 632, 948, 771, 943, 801, 985, 476, 532, 535, 929, 943, 837, 565, 375, 854, 174, 698, 820, 710, 566, 464, 997, 551, 884, 844, 830, 916, 970, 965, 585, 631, 785, 632, 892, 954, 803, 764, 283, 477, 970, 616, 794, 911, 771, 797, 776, 686, 895, 721, 917, 920, 975, 984, 996, 471, 770, 656, 977, 922},
                new int[]{85, 95, 14, 72, 17, 3, 86, 65, 50, 50, 42, 75, 40, 87, 35, 78, 47, 74, 92, 10, 100, 29, 55, 57, 51, 34, 10, 96, 14, 71, 63, 99, 8, 37, 16, 71, 10, 71, 83, 88, 68, 79, 27, 87, 3, 58, 56, 43, 89, 31, 16, 9, 49, 84, 62, 30, 35, 7, 27, 34, 24, 33, 100, 25, 90, 79, 58, 21, 31, 30, 61, 46, 36, 45, 85, 62, 91, 54, 28, 63, 50, 69, 48, 36, 77, 39, 19, 97, 20, 39, 48, 72, 37, 67, 72, 46, 54, 37, 53, 30});
        System.out.println(i);
    }
}
